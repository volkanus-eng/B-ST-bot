...
def ema_hesapla(seri,period):
return seri.ewm(span=period,adjust=False).mean()

def macd_hesapla(seri):
e12=ema_hesapla(seri,12)
e26=ema_hesapla(seri,26)
m=e12-e26
s=ema_hesapla(m,9)
return m-s
...
df[‘mh’]=macd_hesapla(df[‘close’])
df[‘e20’]=ema_hesapla(df[‘close’],20)
...
onceki=df.iloc[-2]
f=son[‘close’]
al=sum([son[‘rsi’]<35,onceki[‘mh’]<0 and son[‘mh’]>0,f>son[‘e20’],son[‘volume’]>son[‘ho’]*1.2])
sat=sum([son[‘rsi’]>65,onceki[‘mh’]>0 and son[‘mh’]<0,f<son[‘e20’],son[‘volume’]>son[‘ho’]*1.2])
if al>=3:
return {‘sembol’:sembol.replace(’.IS’,’’),‘sinyal’:‘AL’,‘puan’:al,‘fiyat’:round(f,2),‘stop’:round(f*0.96,2),‘hedef’:round(f*1.08,2),‘rsi’:round(son[‘rsi’],1)}
if sat>=3:
return {‘sembol’:sembol.replace(’.IS’,’’),‘sinyal’:‘SAT’,‘puan’:sat,‘fiyat’:round(f,2),‘rsi’:round(son[‘rsi’],1)}
...
msg+=‘AL SINYALLERI:\n’
for s in al_list[:5]:
msg+=s[‘sembol’]+’ @ ‘+str(s[‘fiyat’])+’ TL | RSI:’+str(s[‘rsi’])+’ | Stop:’+str(s[‘stop’])+’ | Hedef:’+str(s[‘hedef’])+’\n’
else:
msg+=‘AL: Sinyal yok\n’
if sat_list:
msg+=’\nSAT SINYALLERI:\n’
for s in sat_list[:3]:
msg+=s[‘sembol’]+’ @ ‘+str(s[‘fiyat’])+’ TL | RSI:’+str(s[‘rsi’])+’\n’
tg(msg)
print(‘Bitti!’)
print(msg)
